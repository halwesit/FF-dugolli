...
class DugolliSupabaseUser extends BaseAuthUser {
  DugolliSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> dugolliSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = DugolliSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
